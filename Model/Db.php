...
class Pektsekye_Ymm_Model_Db
{
    ...
    public function __construct() {
			...
			$this->_wpdb = $wpdb;   
      $this->_mainTable = "{$wpdb->prefix}ymm"; 
     ...
     public function getVehicleData($limit = 0)     
    {
      ...
      $select = "
        SELECT DISTINCT posts.post_title, ymm.make, ymm.model, ymm.year_from, ymm.year_to  
    ...
    public function getProductIdsBySku($limit = 0)
    {
      ...
      $select = "
        ...
        FROM {$this->_wpdb->posts} AS posts 
		    ...
		    WHERE posts.post_type IN ('product') AND postmeta.meta_key = '_sku'
		    {$queryLimit}          
      ";
    ...
    public function getProductIdsByTitle()
    {
      $select = "
      ...
      $productIds = array();
      foreach ($result as $row){
        $shortTitle = strtolower(preg_replace('/[\s\W]+/', '', $row['post_title']));
        $productIds[$shortTitle] = $row['ID'];
      }
