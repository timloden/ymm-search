...
class Pektsekye_Ymm_Model_Db_CsvImportHandler
{
    ...
    public function importFromCsvFile($file, $mode)
    {
      ...
      $productIdsByTitle = $this->_db->getProductIdsByTitle();
      ...
      foreach ($rows as $rowIndex => $row) {
        ...
        $productTitle = $d['product_name'];
        if (empty($productTitle)){
          Pektsekye_YMM()->setMessage(sprintf(__('Row #%d was not imported. The "product_name" field should not be empty.', 'ymm-search'), $rowIndex), 'error_lines');
          continue;                  
        }

        $shortTitle = strtolower(preg_replace('/[\s\W]+/', '', $productTitle));
        if (!isset($productIdsByTitle[$shortTitle])){
          Pektsekye_YMM()->setMessage(sprintf(__('Row #%d was not imported. The product with title "%s" does not exist.', 'ymm-search'), $rowIndex, $productTitle), 'error_lines');
          continue;          
        }
      
        $d['product_name'] = $productIdsByTitle[$shortTitle]; //save product id instead of product name in the database 
